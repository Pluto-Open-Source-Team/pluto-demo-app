                    ...
                    if (_thisPolicy.valueStructure.policySchema === policySchemasDataResponse[k].schemaName) {
                        ...
                        if (policySchemasDataResponse[k].notices) {
                            for (let l = 0; l < policySchemasDataResponse[k].notices.length; l++) {
                                if (policySchemasDataResponse[k].notices[l].acknowledgementRequired === true) {
                                    let ackNotice = '';

                                    for (let m = 0; m < policySchemasDataResponse[k].fieldDescriptions.length; m++) {
                                        let _field = policySchemasDataResponse[k].fieldDescriptions[m].field;

                                        if (_field.toLowerCase().includes('acknotice') && _field.toLowerCase().includes(policySchemasDataResponse[k].notices[l].field.toLowerCase())) {
                                            ackNotice = _field;
                                            break;
                                        }
                                    }
